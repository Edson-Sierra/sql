...
customer_id
,market_date
,ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY  market_date ) as "number of visits" 
...
SELECT 
     customer_id
    ,market_date
    ,DENSE_RANK() OVER (PARTITION BY customer_id ORDER BY market_date) AS "number of visits"
FROM customer_purchases
WHERE market_date < '2022-04-29';
...
FROM ( 
	...
	,ROW_NUMBER() OVER (ORDER BY SUM (quantity * cost_to_customer_per_qty) DESC) AS highest_total_sales
	FROM customer_purchases 
...
FROM ( 
	...
	market_date
	,SUM (quantity * cost_to_customer_per_qty) AS total_sales
	,ROW_NUMBER() OVER (ORDER BY SUM (quantity * cost_to_customer_per_qty) ASC) AS lowest_total_sales
	FROM customer_purchases 
	GROUP BY market_date

)

WHERE lowest_total_sales = 1;
